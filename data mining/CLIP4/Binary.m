function Rule=Binary(DatasetName,Decision)
%read data;
...
while best_covered>=0
      ...
      POS=tempPOS(row,:);
      %stop criterion;        
      if best_covered<previous_best_covered && best_covered<NumPos/2
         best_covered=-1;
      end
      %assign new value;
    ...
    for j=1:sz(2)
        if j<sz(2)
           tempRule=[tempRule,attribute{result{i}{j}{1}},'#',result{i}{j}{2},' ','AND',' ']; 
                ...
                tempRule=['IF',' ',tempRule,' ','THEN',' ',attribute{end},'=',Decision];
             end
        end
    end
    Rule{i}=tempRule;
